...
class DownloadStatsProgress:
    ...
    async def get_progress(self) -> Panel:
        ...
        return Panel(self.progress_group, title="下载失败情况", border_style="green", padding=(1, 1))
    ...
    async def add_failure(self, failure_type: [str, int]) -> None:
        ...
        if isinstance(failure_type, int):
            failure_type = str(failure_type) + " HTTP状态"
...
class ScrapeStatsProgress:
    ...
    async def get_progress(self) -> Panel:
        """Returns the progress bar"""
        return Panel(self.progress_group, title="爬取失败统计", border_style="green", padding=(1, 1))

...
def edit_authentication_values_prompt(manager: Manager) -> None:
    ...
    while True:
        console.clear()
        console.print("编辑身份验证值")
        action = inquirer.select(
            message="您想做什么？",
            choices=[
                Choice(1, "编辑 DDOS-Guard Cookies"),
                Choice(2, "编辑论坛身份验证值"),
                Choice(3, "编辑 JDownloader 身份验证值"),
                Choice(4, "编辑 Reddit 身份验证值"),
                Choice(5, "编辑 GoFile API 密钥"),
                Choice(6, "编辑 Imgur 客户端 ID"),
                Choice(7, "编辑 PixelDrain API 密钥"),
                Choice(8, "完成"),
        ...
        # Edit GoFile API Key
            ...
            gofile_api_key = inquirer.text(
                message="请输入 GoFile API 密钥：",
                default=auth["GoFile"]["gofile_api_key"],
                long_instruction="您可以从 https://gofile.io/myProfile 获取您的高级 GoFile API 密钥",
            ).execute()
        ...
        # Edit Imgur Client ID
            ...
            imgur_client_id = inquirer.text(
                message="请输入 Imgur 客户端 ID：",
                default=auth["Imgur"]["imgur_client_id"],
                long_instruction="您可以创建一个应用程序并获取您的客户端 ID "
                                 "从 https://imgur.com/account/settings/apps"
            ).execute()
        ...
        # Edit PixelDrain API Key
            ...
            pixeldrain_api_key = inquirer.text(
                message="请输入 PixelDrain API 密钥：",
                default=auth["PixelDrain"]["pixeldrain_api_key"],
                long_instruction="您可以从 https://pixeldrain.com/user/api_keys 获取您的高级 API 密钥",
            ).execute()
